...
{
    std::unique_lock<std::mutex> lock1(mtx_vehicles);
    return _vehicles.size();
}

void WaitingVehicles::pushBack(std::shared_ptr<Vehicle> vehicle, std::promise<void> &&promise)
{
    std::lock(mtx_vehicles, mtx_promises);
    std::unique_lock<std::mutex> lock1(mtx_vehicles, std::adopt_lock);
    std::unique_lock<std::mutex> lock1(mtx_vehicles, std::adopt_lock);
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    std::unique_lock<std::mutex> lock_cout(TrafficObject::_mtxCout);
